...
void ssm2604_init(void)
{
	...
	reg_write(REG_LEFT_CHANNEL_ADC_INPUT_VOLUME, LRINBOTH | LINVOL(0x10));
	reg_write(REG_RIGHT_CHANNEL_ADC_INPUT_VOLUME, RLINBOTH | RINVOL(0x10));
	reg_write(REG_ANALOG_AUDIO_PATH, DACSEL);
	reg_write(REG_DIGITAL_AUDIO_PATH, 0*ADCHPF);
